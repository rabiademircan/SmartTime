...
public class WeeklyOverviewPanel extends JPanel {
    ...
    private void loadWeekData(String whichWeek) {
        ...
        String dayKey = SessionManager.getToday().getDayOfWeek().toString().substring(0, 1)
                + SessionManager.getToday().getDayOfWeek().toString().substring(1, 3).toLowerCase();

        int todayMinutes = log.getOrDefault(dayKey, 0);
        if (weekData.containsKey(dayKey)) {
            weekData.put(dayKey, todayMinutes);
        }
    }
